...
--Soru 2: En yüksek fiyatlı ürüne sahip olan ilk 5 kategori hangisidir?
...
--Soru 3:En çok satın alınan ürünler ve kategorileri nelerdir?
...
--Soru 3:Nakliye firmalarının geciken toplam teslimat süreleri ve geciken toplam sipariş miktarları nelerdir?


SELECT
    s.company_name AS shipper,
    SUM(o.shipped_date - o.required_date) AS total_delay_days,
...
--Kullanılan nakliye yöntemleri sayıları

SELECT 
    CASE 
        WHEN o.ship_via = 1 THEN 'Air'
        WHEN o.ship_via = 2 THEN 'Land'
        WHEN o.ship_via = 3 THEN 'Sea'
    END AS shipping_method,
    COUNT(o.order_id) AS total_orders
FROM 
    public.orders o
WHERE 
    o.ship_via IN (1, 2, 3)  -- Sadece kara, deniz ve hava taşıma yöntemlerini filtrele
GROUP BY 
    shipping_method
ORDER BY 
    total_orders DESC;

	-- iNTERNETTEN EDİNDİĞİM BİLGİLERE GÖRE NAKLİYE TİPLERİNDE EN ÇOK KULLANILAN 3 YÖÖNTEM SIRASIYLA 
-- KARA, DENİZ VE HAVA'DIR. BU YÜZDEN VERİLEN VERİSETİNDE EN ÇOK KULLANILAN NAKLİYE YÖNTEMİNE KARA 
-- İKİNCİSİNE DENİZ, ÜÇÜNCÜSÜNE İSE HAVA KODLAMASINI ATADIM.


--MÜŞTERİ-SİPARİŞ ANALİZİ
--Hangi müşteriler daha fazla sipariş veriyor?
...
-- Soru 2: Müşterilerin siparişlerdeki toplam harcamaları ne kadar?
...
--ÇALIŞAN ANALİZİ
--SORU 1 : Çalışan başına satış miktarı nedir?
...
-- Soru 2: Çalışanlar toplam kaç müşteri ile ilgilenmiştir?
...
SELECT 
    c.customer_id,
    c.company_name,
    EXTRACT(DAY FROM age(CURRENT_DATE, co.last_order_date)) AS recency,
    co.total_orders AS frequency,
    ROUND(co.total_spent::numeric, 2) AS monetary
...
rfm_scores AS (
    SELECT 
        ...
        -- Recency Puanlama
        CASE 
            WHEN EXTRACT(DAY FROM age(CURRENT_DATE, co.last_order_date)) >= 26 THEN 1
            WHEN EXTRACT(DAY FROM age(CURRENT_DATE, co.last_order_date)) >= 21 THEN 2
            WHEN EXTRACT(DAY FROM age(CURRENT_DATE, co.last_order_date)) >= 16 THEN 3
            WHEN EXTRACT(DAY FROM age(CURRENT_DATE, co.last_order_date)) >= 11 THEN 4
            ELSE 5 
        END AS recency_score,
        -- Frequency Puanlama
        CASE 
            WHEN co.total_orders BETWEEN 2 AND 15 THEN 1
            WHEN co.total_orders BETWEEN 16 AND 30 THEN 2
            WHEN co.total_orders BETWEEN 31 AND 60 THEN 3
            WHEN co.total_orders BETWEEN 61 AND 90 THEN 4
            ELSE 5 
        END AS frequency_score,
        -- Monetary Puanlama
        CASE 
            WHEN co.total_spent <= 1000 THEN 1
            WHEN co.total_spent <= 5000 THEN 2
            WHEN co.total_spent <= 20000 THEN 3
            WHEN co.total_spent <= 75000 THEN 4
            ELSE 5 
...
--İskonto uygulanan müşteriler 

SELECT
    COUNT(DISTINCT c.customer_id) AS total_discounted_customers,
    c.customer_id,
    c.company_name,
    c.contact_name
FROM
...
ORDER BY
    total_discounted_customers DESC;
